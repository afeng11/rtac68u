...
void tm1900_check(){
	//tm1900->ac68u
	if(!strcmp(cfe_nvram_get("odmpid"), "TM-AC1900")){
		system("dd if=/rom/cfe/tm1900.bin of=/dev/mtdblock0 2>/dev/null");
		sleep(2);
		merlinr_set("odmpid", "ASUS");
		merlinr_set("secret_code", nvram_safe_get("secret_code"));
		merlinr_set("et0macaddr", nvram_safe_get("et0macaddr"));
		merlinr_set("0:macaddr", nvram_safe_get("0:macaddr"));
		merlinr_set("1:macaddr", nvram_safe_get("1:macaddr"));
		doSystem("mtd-erase2 asus");
	}
}
#endif
